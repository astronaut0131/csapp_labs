...
void error_wrapper(int ret, char *msg);
...
/*
 * error_wrapper - wrapper function that checks ret value
 */
void error_wrapper(int ret, char *msg) {
	if (ret < 0) unix_error(msg);
}
...
void do_bgfg(char **argv) 
{
	...
	if (job->state == ST)
		error_wrapper(kill(job->pid * -1,SIGCONT),"kill error");
	if (!strcmp(argv[0],"fg")) {
		if (job->state != ST && job->state != BG) {
			printf("fg command can only be applied to stopped or background job\n");
		} else {
			job->state = FG;
			waitfg(job->pid);
		}
		...
		if (job->state != ST)
			printf("bg command can only be applied to stopped job\n");
		else {
...
void waitfg(pid_t pid)
{
	while(getjobpid(jobs,pid) && (getjobpid(jobs,pid))->state != ST)
		sleep(1);	
}
